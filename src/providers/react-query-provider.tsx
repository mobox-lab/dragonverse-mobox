...
interface test____add_API_contract_testsProps {
  title?: string;
  onSuccess?: (result: any) => void;
  onError?: (error: Error) => void;
}

interface test____add_API_contract_testsState {
  isLoading: boolean;
  data: any;
  error: Error | null;
}

export const usetest____add_API_contract_tests = () => {
  const [state, setState] = useState<test____add_API_contract_testsState>({
    isLoading: false,
    data: null,
    error: null
  });

  const handletest____add_API_contract_tests = useCallback(async () => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
    
    try {
      const result = await apiCall('/test____add_API_contract_tests');
      setState(prev => ({ ...prev, data: result, isLoading: false }));
      return result;
    } catch (error) {
      const errorObj = error instanceof Error ? error : new Error('Unknown error');
  ...
  return {
    ...state,
    handletest____add_API_contract_tests
  };
};
